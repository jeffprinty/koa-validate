...
describe('koa-validate', function() {
	it('json path basic', function(done) {
		...
		app.router.post('/json', ctx => {
			...
			ctx
				.checkBody('/store/book[0]/disabled', true)
				.first()
				.notEmpty()
				.toBoolean();
			ctx
				.checkBody('#/store/book[0]/category', true)
				.first()
				.trim()
				.eq('reference');
			ctx
				.checkBody('/store/book[*]/price', true)
...
describe('koa-validate type', function() {
	it('type check', function(done) {
		...
		app.router.post('/json', ctx => {
			ctx.checkBody('/', true).notEmpty();
			ctx
				.checkBody('/store/book[0]/price', true)
				.get(0)
				.type('number')
				.type('primitive');
			...
			// ctx
			// 	.checkBody('/store/book[0]/publishDate', true)
			// 	.get(0)
			// 	.toDate()
			// 	.type('date')
			// 	.type('object');
	...
	it('type fail check', function(done) {
		...
		app.router.post('/json', ctx => {
			...
			// 	.toDate()
			// 	.type('array');
			if (ctx.errors && 4 == ctx.errors.length) {
				ctx.status = 200;
			} else {
				ctx.status = 500;
			}
