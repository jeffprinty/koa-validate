...
module.exports = function(app) {
	app.context.checkQuery = function(key, transFn) {
		return new Validator(
			this,
			key,
			getValue(this.request.query, key, transFn),
			hasKey(this.request.query, key, transFn),
			this.request.query
		);
	};
	...
	app.context.checkBody = function(key, transFn) {
		...
		return new Validator(
			this,
			key,
			getValue(body, key, transFn),
			hasKey(body, key, transFn),
			body
		);
	};
	app.context.checkFile = function(key, deleteOnCheckFailed) {
		if (
			'undefined' == typeof this.request.body ||
			'undefined' == typeof this.request.body.files
		) {
			if (!this.errors) {
		...
		deleteOnCheckFailed = 'undefined' == typeof deleteOnCheckFailed
			? true
			: false;
...
Validator.prototype.notBlank = function(tip) {
	if (
		...
		(null == this.value ||
			'undefined' == typeof this.value ||
			('string' == typeof this.value && /^\s*$/gi.test(this.value)))
	) {
...
Validator.prototype.isLength = function(min, max, tip) {
	...
	if (this.goOn) {
		if (this.value.length < min) {
			this.addError(
				tip || this.key + "'s length must equal or great than " + min + '.'
			);
			return this;
		}
		if (-1 != max && this.value.length > max) {
			this.addError(
				tip || this.key + "'s length must equal or less than " + max + '.'
			);
			return this;
		}
	}
...
Validator.prototype.isAlphanumeric = function(tip, locale) {
	if (
		this.goOn &&
		(!isString(this.value) || !v.isAlphanumeric(this.value, locale))
	) {
...
Validator.prototype.isDate = function(tip) {
	if (
		this.goOn &&
		!isDate(this.value) &&
		(!isString(this.value) || !isDate(this.value))
	) {
...
Validator.prototype.isCurrency = function(tip, options) {
	if (
		this.goOn && (!isString(this.value) || !v.isCurrency(this.value, options))
	) {
...
Validator.prototype.isMobilePhone = function(tip, locale) {
	if (
		this.goOn && (!isString(this.value) || !v.isMobilePhone(this.value, locale))
	) {
...
Validator.prototype.isFQDN = function(tip, options) {
	if (this.goOn && (!isString(this.value) || !v.isFQDN(this.value, options))) {
		this.addError(
			tip || this.key + ' is not a fully qualified domain name format.'
		);
	}
	return this;
};
...
Validator.prototype.encodeBase64 = function() {
	if (this.goOn && !this.hasError() && this.value) {
		this.value = this.params[this.key] = new Buffer(this.value).toString(
			'base64'
		);
	}
	return this;
	...
	if (!this.hasError() && this.value) {
		try {
			if (inBuffer) {
				...
				this.value = this.params[this.key] = new Buffer(
					this.value,
					'base64'
				).toString();
			}
...
Validator.prototype.hash = function(alg, enc) {
	if (!this.hasError() && this.value) {
		enc = enc || 'hex';
		this.value = this.params[this.key] = require('crypto')
			.createHash(alg)
			.update(this.value)
			.digest(enc);
	}
...
Validator.prototype.clone = function(key, value) {
	if (!this.hasError() && this.value) {
		this.value = this.params[key] = 'undefined' == typeof value
			? this.value
			: value;
		this.key = key;
	}
...
Validator.prototype.check = function(fn, tip, scope) {
	if (
		this.goOn &&
		!this.hasError() &&
		!fn.call(scope || this, this.value, this.key, this.context)
	) {
...
Validator.prototype.type = function(t, tip) {
	if (this.value) {
		if (
			...
			if (t != typeof this.value)
				this.addError(tip || this.key + ' is not ' + t + '');
		} else if ('array' == t) {
			if (!util.isArray(this.value))
				this.addError(tip || this.key + ' is not array');
		} else if ('date' == t) {
			if (!isDate(this.value)) this.addError(tip || this.key + ' is not date.');
		} else if ('null' == t) {
			if (!util.isNull(this.value))
				this.addError(tip || this.key + ' is not null.');
		} else if ('nullorundefined' == t.toLowerCase()) {
			if (!util.isNullOrUndefined(this.value))
				this.addError(tip || this.key + ' is not primitive type.');
		} else if ('primitive' == t) {
			if (!util.isPrimitive(this.value))
				this.addError(tip || this.key + ' is not primitive type.');
		} else {
...
FileValidator.prototype.size = function(min, max, tip) {
	if (
		this.goOn && (!this.value || this.value.size < min || this.value.size > max)
	) {
		this.addError(
			tip ||
				'file ' +
					((this.value && this.value.name) || this.key) +
					"' length must between " +
					formatSize(min) +
					' and ' +
					formatSize(max) +
...
FileValidator.prototype.suffixIn = function(arr, tip) {
	if (
		...
		(!this.value ||
			-1 ==
				arr.indexOf(
					-1 == this.value.name.lastIndexOf('.')
						? ''
						: this.value.name.substring(this.value.name.lastIndexOf('.') + 1)
				))
	) {
		this.addError(
			tip ||
...
FileValidator.prototype.copy = function*(dst, afterCopy) {
	if (this.goOn && this.value) {
		...
		if (
			dstFile.length - 1 == dstFile.lastIndexOf('/') ||
			dstFile.length - 1 == dstFile.lastIndexOf('\\') ||
			((yield coFsExists(dstFile)) && (yield coFsIsDir(dstFile)))
		) {
